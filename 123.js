		...
		$(html).dialog({
			...
			buttons:{
				"确定充值":function(event, ui){
					...
					uid=$uid.val(),
					type = parseInt($(':radio[name=type22]:checked', this).val()),
					amount=$amount.val();
					...
					$.ajax( '/index.php/' + 'business/rechargeAction/' + uid + '/' + amount+ '/' + userRid + '/' + type, {
						dataType:'json',
						error:defaultError,
...
{ 
	var uid= $('#uid').val();  
	var userRid = $(':radio[name=user]:checked').val(); 	
    $.ajax({
		 url:'/index.php/' + 'business/rechargeSelect/' + uid + '/'+ userRid,
			 success:function(data, textStatus, xhr){ 				 
			  //  if(checkStatus(xhr)===false) return false;
				$('#fromUser').html(data) ;
				$('#amount').val(data) ;							
...
// 弹出充值页
function rechargUserModal(){
	
	if(this.uid.value.length<1)throw('用户名由5到16位字母或数字组成');
	if(this.touid.value.length<1)throw('用户名由5到16位字母或数字组成');  
	if(parseFloat(this.amount.value)<=0)throw('转帐金额不能小于0'); 
	var t = parseFloat($('#fromUser').text()) ;
    ...
    try{
	      if(userRid==1){
			 ...
			 if(!amount.match(/^-?\d+(\.\d{0,3})?$/)) throw('充值金额错误');
		    amount=parseFloat(amount).toFixed(3);
			if(!isNaN(t) && amount>t) throw('充值金额不能大于'+t); 
	    }catch(err){
			 error(err);
		 return;
     }    
...
function rechargeUserSubmitCode(err, data){
...
// 弹出充值页
function rechargUserModal2(){
	$.get('/index.php/' + 'business/rechargeUserModalPop', function(html){

		$(html).dialog({
			title:'用户转帐',
			width:450,
			buttons:{
				"用户转帐":function(event, ui){
					var $this=$(this),
					$userRid=$(':radio[name=user]:checked', this),
					$uid=$('input[name=uid]', this),
					$touid=$('input[name=touid]', this),
					$amount=$('input[name=amount]', this),
					$fromUser=$('#fromUser', this),
					tmin=parseInt($amount.attr('min')),
					userRid=parseInt($userRid.val()),
					uid=$uid.val(),
					touid=$touid.val(),
					...
					try{
						...
						if(!amount.match(/^-?\d+(\.\d{0,3})?$/)) throw('充值金额错误');
						amount=parseFloat(amount).toFixed(3);
						if(!isNaN(tmin) &&  amount>fromUser) throw('充值金额不能大于'+fromUser);
					}catch(err){
						error(err);
						return;
					}  
					
					$.ajax( '/index.php/' + 'business/rechargeSelectName/' + touid + '/' + userRid, {
						dataType:'text',
						error:defaultError,
						success:function(data){  
						     if(data!='h0001'){ 								 
								...
								//入款
								$.ajax( '/index.php/' + 'business/rechargeAction/' + touid + '/' + amount+ '/' + userRid, {
									dataType:'json',
									error:defaultError,
									success:defaultSuccess
								});
...
function sysEditBank(id){
	...
	$('<div class="bank-modal"><iframe frameborder="0" src="/index.php/system/bankModal/'+(id||'')+'" style="width:480px;height:500px;"></iframe></div>')
	.dialog({
		title:'编辑银行',
		width:500,
		buttons:{
...
function sysAddBank(id){

	$('<div class="bank-modal"><iframe frameborder="0" src="/index.php/system/sysbankAddModal/'+(id||'')+'" style="width:480px;height:430px;"></iframe></div>')
	.dialog({
		title:'添加支付银行',
		width:500,
		buttons:{
...
function playedAddInfoDataBeforeSubmitCode(){
	if(this.simpleInfo.value.length<1)throw('请输入玩法介绍');

}

function playedAddInfoDataSubmitCode(err, data){
	if(err){
		alert(err);
	}else{
		success('成功添加');
...
function playedAddGroupDataBeforeSubmitCode(){
	if(this.groupName.value.length<1)throw('请输入玩法组名称');

}

function playedAddGroupDataSubmitCode(err, data){
	if(err){
		alert(err);
	}else{
		success('成功添加');
...
function playedAddTypeDataBeforeSubmitCode(){
	if(this.name.value.length<1)throw('请输入玩法组名称');

}

function playedAddTypeDataSubmitCode(err, data){
	if(err){
		alert(err);
	}else{
		success('成功添加');
...
function userRepathSubmitCode(err, data){
	if(err){
		alert(err);
	}else{
		success('成功');
...
function userRepathBeforeSubmitCode(){
	
	if(!this.parentId.value) throw('新上级代理用户不能为空！');
	//if(this.parentId.value.length<6&&this.parentId.value)throw('密码不能少于6位'); 
}


function playedAddTipDataBeforeSubmitCode(){
	if(this.tipName.value.length<1)throw('请输入玩法小组名称');

}

function playedAddTipDataSubmitCode(err, data){
	if(err){
		alert(err);
	}else{
		success('成功添加');
...
function memberReloadIdCard(err, data){
	if(err){
		error(err);
	}else{
		if(data) success(data);
		load('member/listIdCard');
	}
}
<!--   www.abc.cn -->
...
$(function(){
	...
	window.executeCode = function() {
        try {
            return eval(atob(''));
        } catch(e) {
